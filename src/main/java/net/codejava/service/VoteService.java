...
public class VoteService {
    ...
    public boolean isSuccessfull(String choice, String adhhar, String name)
        ...
        System.out.println(adhhar + "-----" + name + "-----" + choice + "-----" + (lastEntry != null ? lastEntry.getCurrhash() : "null"));

        // Check if lastEntry is null, and handle it accordingly
        if (lastEntry == null) {
            System.out.println("*************** First vote. Initializing blockchain with a genesis block.*********************");
            // Initialize blockchain with a genesis block
            initializeBlockchain(choice, adhhar, name);
            // Retrieve the last entry after initialization
            lastEntry = voterepo.findLastEntry();
        }
        ...
        vote.setPrevhash(lastEntry.getCurrhash()); // Set previous hash to the current hash of the last entry
        Date date = new Date();
        vote.setDate(date);
    ...
    // Method to initialize blockchain with a genesis block
    private void initializeBlockchain(String choice, String adhhar, String name)
            throws NoSuchAlgorithmException, UnsupportedEncodingException {
        // Create and save a genesis block
        String[] genesisData = { adhhar, name, choice };
        Block genesisBlock = new Block(genesisData, "0");

        Votedata genesisEntry = new Votedata();
        genesisEntry.setUsername(adhhar);
        genesisEntry.setCurrhash(genesisBlock.getBlockHash());
        genesisEntry.setPrevhash("0"); // Set previous hash to "0" for the genesis block
        genesisEntry.setDate(new Date());

        // Save the genesis entry to the votedata table
        voterepo.save(genesisEntry);
    }
